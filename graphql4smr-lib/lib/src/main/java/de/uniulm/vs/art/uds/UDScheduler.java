...
public class UDScheduler {
    ...
    private long numberOfThreadsScheduledtotal;
     ...
     */
    private final ThreadLocal<UDSThread> currentUDSThread = new ThreadLocal<>();
     ...
     */
    private final UDSConfiguration udsConfiguration; //= new UDSConfiguration(1);
     ...
     */
    public UDSThread getCurrentUDSThread() {
    ...
    public void addRequest(Runnable r, Runnable replyRunnable) {
        ...
        try {
             ...
             */
            UDSThread thread = new UDSThread(this);
        ...
        try {
            ...
            for(i = 0, n = 0; true; i++) {
                ...
                if(!t.isTerminated() && prim(t)) {
                    ...
                    //long tempvalue = Math.min(udsConfiguration.getN(),numberOfThreadsScheduledtotal - numberOfThreadsTerminated);
    ...
    public void checkForEndOfRound() {
        ...
        try {
            ...
            //System.out.println(prefix + "end of round " + round + " detected");
    ...
    private void reconfigure(boolean progress) {
        schedulerLock.lock();
        String prefix = currentUDSThread.get().getIdString() + " reconfigure(): ";
        if(logger.isLoggable(Level.FINE)) {
    ...
    protected void terminateThread(UDSThread t) {
        ...
        try {
            ...
            //System.out.println(t.getIdString() + " terminateThread(): before waitForPrimary()");
            ...
            removeFromOrder(t);
            //System.out.println("terminated 1 "+numberOfThreadsTerminated);

            numberOfThreadsTerminated++;

            checkForEndOfRound();

            //System.out.println("terminated 2 "+numberOfThreadsTerminated);
    ...
    protected class UDSThread {
        ...
        private final UDScheduler uds;

        /**
         * Constructor of UDS threads
         */
        UDSThread(UDScheduler uds) {
            this.uds = uds;
